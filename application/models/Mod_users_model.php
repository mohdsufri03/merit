...
class Mod_users_model extends CI_Model {
	
	function login($username, $password){
		$this->db->flush_cache();
		
		$this->db->where('mod_users_username',$username);
		$this->db->where('mod_users_password',md5($password));
		
		$this->db->join('mod_groups','mod_groups.mod_groups_id = mod_users.mod_users_id');
		$query = $this->db->get('mod_users');
